...
export default function Item(props) {
  ...
  const getRecords = () => {
    axios
      .get(`${process.env.EC2_server}/records/${id}`)
      .then((res) => {
  ...
  const handleArticleDeletion = () => {
    axios
      .delete(`${process.env.EC2_server}/records/${id}`, { withCredentials: true })
  ...
  const handleArticleEditComplete = () => {
    ...
    axios
      .put(
        `${process.env.EC2_server}/records/${id}`,
        {
          title: post.title,
  ...
  const handleSubmitButton = () => {
    if (editingComment) {
      axios
        .patch(
          `${process.env.EC2_server}/comments/${editingComment}`,
          { content: text },
          { withCredentials: true }
        )
        .then((res) => console.log(res))
      ...
      axios
        .post(
          `${process.env.EC2_server}/comments/${id}`,
          { content: `${text}` },
          { withCredentials: true }
        )
        .then((res) => console.log(res))
  ...
  const handleCommentDeletion = (commId) => {
    axios.delete(`${process.env.EC2_server}/comments/${commId}`).then((res) => console.log(res));
  };
