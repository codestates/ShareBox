...
export default function App() {
  ...
  const handleSearch = () => {
    if (keyword) {
      let search = axios
        .get(
          `${
            process.env.EC2_server
          }/search?search_type=title&title=${keyword}&page=${1}&limit=${100}`
        )
        .then((res) => {
  ...
  const handleDropout = () => {
    axios.post(`${process.env.EC2_server}/dropout`).then((res) => {
      setSignedIn(false);
      navigate("/");
    });
  };
  ...
  const getData = () => {
    ...
    axios
      .get(`${process.env.EC2_server}/main`)
      .then((res) => {
